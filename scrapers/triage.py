...
The harvester (scrapers/harvest.py) pulls every board whole and stores
...
Gates, per configured track, in order:
  ...
  1. mission   company-level, once per COMPANY: the roster row's cached
               mission tier (core.claude.is_active_mission) and the
  ...
  2. title     the track's technical-title regex (core.gates), title only.
  ...
  4. geo       the track's geo_gate, judged on the location FIELD: local
               ...
               trusted company.
  5. exclude   the track's [exclude.<id>] tables (core.gates).
  6. division  multi-division companies only: core.filters.is_relevant on
               the body (needs one).
  --- hydrate survivors (the harvester's per-host caps and pauses) ---
  gates 3-6 again, now with a body
  7. fit       Claude fit score (core.fit); the verdict is 'fit' when the
...
# default when no place was found (core.locality.location_snippet).
...
def ensure_mission(conn, company, titles=(), scorer=score_company_mission):
    ...
    policy belongs to discovery.local_sourcing.score_missions. Returns
