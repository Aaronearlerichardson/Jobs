...
The stages, and where each lives:
  ...
  1. Gather candidate names -- profile seeds, directory scrapes, web-search
     harvesting, an LLM brainstorm (discovery.name_sources) -- or take them
     from a pasted page (discovery.paste_ingest) or a banked lead
     (resolve_leads below).
  2. Resolve each name to a board: careers-page sniff first
     (discovery.sniffer), name-guessed slug probes second (probe_company /
     discovery.probes), web search last (discovery.websearch_board), every
...
# --------------------------------------------------------------------------- #
#  Board probing and discover_local (slug guesses live in core/names.py)      #
# --------------------------------------------------------------------------- #
