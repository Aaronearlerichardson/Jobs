...
both into the names the rest of the code reads:
    ...
    config/secrets.py   env vars: API keys, model names            (leaf)
    config/paths.py     SCRIPT_DIR / APP_HOME / DATA_DIR, store     (leaf)
    config/profile.py   profile.toml loaded; keywords, locations,
                        candidate, résumé, fit, mission, locality,
                        discovery; `profile_section(name)`
    config/tracks.py    [tracks.*] tables + engine defaults -> UI_TRACKS
    config/policy.py    [policy] + HTTP timeouts / user agents
    config/sources.py   [sources]: forums, web search, aggregator feeds
...
attribute at use time — scrapers/runner.py and webapp/ops.py reassign them
