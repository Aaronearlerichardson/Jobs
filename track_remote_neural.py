...
existing lightweight sources for general neural-ML roles. Only postings
that pass remote-eligibility detection are surfaced, and every digest entry
is tagged [REMOTE-NEURAL].

Usage:
    python track_remote_neural.py                 # PREVIEW: live fetch, no
                                                  #   DB writes, no email.
                                                  #   Prints per-source counts
                                                  #   + sample matches.
    python track_remote_neural.py --commit        # also persist matches
                                                  #   (remote_eligible=1) to
                                                  #   the seen-jobs DB
    python track_remote_neural.py --send          # also email the tagged
                                                  #   digest (requires creds)
    python track_remote_neural.py --no-websearch  # skip the (flaky) DDG
                                                  #   web-search sources
    python track_remote_neural.py --samples 8     # show N sample matches
...
def main():
    ...
    ap.add_argument("--commit", action="store_true",
                    help="Persist remote-eligible matches to the seen-jobs DB")
